...
**図書館目次 OCR × AI構造化 デスクトップアプリケーション**

国立国会図書館（NDL）が開発した **[NDLOCR-Lite](https://github.com/ndl-lab/ndlocr-lite)** をベースに、橋本雄太氏（国立歴史民俗博物館）の **[ndlocrlite-web](https://github.com/yuta1984/ndlocrlite-web)** をフォークし、宮川創氏（筑波大学）によるUI拡張機能を取り込んだ上で、図書館向けパイプライン機能を追加したTauriデスクトップアプリケーションです。

ISBN入力から始まる目次PDF自動取得・OCR・ローカルLLMによる構造化・human-in-the-loopレビュー・JSONL出力までの一連のパイプラインを提供し、図書館資料の目次データを検索可能な形に変換します。また、スタンドアロンでのOCRタブ機能も備えており、AI（Claude、GPT、Gemini等）による校正機能で歴史的文書のデジタル化精度を向上させます。

## 機能

### パイプラインモード（Tauri デスクトップ）

- **ISBN → SRU → PDF自動取得** — ISBNまたはCSVで複数の図書の目次PDFを自動ダウンロード（SRUサーバー経由）。MARC-XMLから880リンク解決・856選別で目次PDFを特定
- **目次PDF → OCR → LLM構造化** — ダウンロード済みPDFを自動OCR処理後、ローカルLLM（Ollama互換）で見出し・ページ番号の構造を自動抽出。非TOCページ事前除外で精度向上
...
### パイプラインモード（目次データベース構築）

1. **Inboxタブ** でISBN入力
   - 単体入力：ISBN欄に入力して追加
   - CSV一括：複数ISBNをCSVでアップロード
2. **SRU解決** でPDF URL特定
   - 自動でSRUサーバーに問い合わせ（国立国会図書館等）
   - MARC-XMLから目次PDF URLを抽出
3. **PDF取得** で目次ファイル保存
4. **OCR処理** で画像化
5. **LLM構造化** で見出し＆ページ番号を抽出
6. **Reviewタブ** で Human-in-the-loop レビュー
   - 自動構造化結果を確認・編集
   - ページフィルタで特定ページのみ表示
   - テーブル・Markdown形式で切替表示
7. **JSONL出力** で検索可能なデータに変換

設定パネルで以下をカスタマイズ可能：
- **ローカルLLM設定** — Ollama互換API URL、モデル名、useVisionフラグ
- **出力先** — JSONL保存フォルダ

### OCRタブ（単独OCR処理）

1. アプリを起動
2. 画像（JPG/PNG/TIFF/HEIC）またはPDFをドラッグ＆ドロップするか、クリックして選択
3. 文書言語を選択（日本語がデフォルト）
...
> **Note:** ONNXモデルはアプリに同梱済み（`src-tauri/resources/models/`）。開発時も別途ダウンロードは不要です。
...
- **ONNXモデル同梱** — 高精度レイアウト検出・文字認識に必要なONNXモデル（計約146MB）はアプリに同梱済みです。別途ダウンロードは不要
- **パイプラインモード** — ローカルLLM（Ollama等）の事前セットアップが必要です。設定パネルでエンドポイントとモデルを指定してください
- **OCRタブ** — 処理時間はハードウェア性能に依存します（GPU加速なしのCPU推論のため、1枚あたり数十秒かかる場合があります）
- **AI校正機能** — 使用するには各AIプロバイダのAPIキーが必要です。APIキーはアプリ内で暗号化して保存されます。外部サーバーには送信されません
...
本ツールは以下のプロジェクトの派生物です。ライセンス条件に従い、帰属表示を維持しています。

- **[NDLOCR-Lite](https://github.com/ndl-lab/ndlocr-lite)** — 国立国会図書館によるOCRエンジン（CC BY 4.0）
  - **DEIMv2** — [ShihuaHuang95/DEIM](https://github.com/ShihuaHuang95/DEIM) — レイアウト検出モデル
  - **PARSeq** — [baudm/parseq](https://github.com/baudm/parseq) — 文字認識モデル
  - **NDLmoji.yaml** — 国立国会図書館 — 日本語文字セット定義
- **[ndlocrlite-web](https://github.com/yuta1984/ndlocrlite-web)** — 橋本雄太氏（国立歴史民俗博物館）によるWeb移植
- **[ndlocr-lite-web-ai-deluxe](https://github.com/somiyagawa/ndlocr-lite-web-ai-deluxe)** — 宮川創氏（筑波大学）によるUI拡張
  - 画像前処理（明るさ・コントラスト・シャープネス・二値化・ノイズ除去・傾き補正・湾曲補正・ページ分割）
  - ダークモード（OS設定追従）
  - 多言語UI対応
  - 縦書き表示モード
...
- fix: `download_pdf` にもタイムアウトを追加（120秒）。目次PDFダウンロードが固まるとパイプラインが復旧不能になっていた問題を修正
- fix: ReviewViewのヘッダーで、出力先の絶対パスを含む長いステータスメッセージ（PDF出力・CiNii出力）が操作ボタン列を画面外へ押し出す問題を修正。メッセージをヘッダー直下の専用行に分離
- fix: CiNii出力でレコード（`cinii_books.jsonl`）の書き込み後にログ（`cinii_export.log`）の追記だけが失敗した場合、実際にはレコードが書けているのに「出力エラー」と表示されてしまう問題を修正。レコードは書けたがログ書き込みに失敗した旨を明示するメッセージに変更（再実行すれば自己修復する）
