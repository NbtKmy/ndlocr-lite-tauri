...
export default function App() {
  ...
  const handleMathRecognize = useCallback(async () => {
    ...
    try {
      ...
      const { loadModel, setModelBaseUrl } = await import('./worker/model-loader')
      const { getModelBaseUrl } = await import('./utils/modelPath')

      const modelBaseUrl = await getModelBaseUrl()
      setModelBaseUrl(modelBaseUrl)
      ...
      const tokenizerRes = await fetch(`${modelBaseUrl}/mfr-tokenizer.json`)
