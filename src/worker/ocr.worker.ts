...
import './onnx-config'
import { loadModel, setModelBaseUrl, setPreloadedBuffer } from './model-loader'
  ...
  async initialize(layoutOnly = false, language: RecognitionLanguage = 'ja'): Promise<void> {
    ...
    try {
      ...
      if (layoutOnly) {
        ...
        // モデル読み込みを Promise.race でタイムアウト対応
        const timeoutPromise = new Promise<never>((_, reject) =>
          setTimeout(() => reject(new Error('Model loading timeout (120s)')), 120000)
        )

        const [layoutModelData, rec30Data, rec50Data, rec100Data] = await Promise.race([
          Promise.all([
        ...
        console.log('[ocr.worker] Initializing rec30...')
        await this.recognizer30.initialize(rec30Data)
        console.log('[ocr.worker] rec30 initialized')
        ...
        console.log('[ocr.worker] Initializing rec50...')
        await this.recognizer50.initialize(rec50Data)
        console.log('[ocr.worker] rec50 initialized')
        ...
        console.log('[ocr.worker] Initializing rec100...')
        await this.recognizer100.initialize(rec100Data)
        console.log('[ocr.worker] rec100 initialized')
      }
...
self.onmessage = async (event: MessageEvent<WorkerInMessage>) => {
  ...
  switch (message.type) {
    case 'INITIALIZE':
      if (message.modelBaseUrl) setModelBaseUrl(message.modelBaseUrl)
      if (message.modelBuffers) {
        for (const [type, buf] of Object.entries(message.modelBuffers)) {
          setPreloadedBuffer(type, buf)
        }
      }
